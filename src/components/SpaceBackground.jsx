...
const SpaceBackground = () => {
  ...
  useEffect(() => {
    ...
    // Refresh the background periodically to prevent fading
    const refreshInterval = setInterval(() => {
      // Force a re-render by updating a canvas property or element
      const canvas = canvasRef.current;
      if (canvas) {
        const ctx = canvas.getContext('2d');
        // Clear any accumulated fade effects
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        
        // Redraw the background elements if needed
        // This depends on how your space background is implemented
      }
